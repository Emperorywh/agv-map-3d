...
- 完整增量：得到可复现、机器可读的 100/200 台、1080p/4K 和冷缓存性能基准工具，并在当前执行环境完成冒烟及当前结果报告；正式目标硬件结论由 GATE-001 给出。同时完成场景观感与原型视觉语言的对齐调优，使默认画质下的车体状态色、路网与节点视觉权重、标签紧凑度达到可交付观感。
- 主要范围：`tests/performance/**`、性能浏览器采集配置、只读诊断指标导出、Mock/相机测试配置、性能脚本、为达到结构预算所需的直接渲染优化，以及 `map-visualization` 与 `fleet-monitoring` 内的外观常量与材质/灯光工厂（`mapAppearance`、`fleetAppearance`、语义材质、名称图集布局、方向光与环境光参数）。
...
- 观感对齐必做（当前有效结论：核心语义均已成立，差距集中在视觉参数）：提升车体主状态色在 ACESFilmic+IBL 下的饱和度（调环境光强度/曝光或材质参数，保持 §5.4 管线与 metalness/roughness 推荐区间不变）；缩小节点实例相对路网的视觉权重（半径/亮度）并提高路面与中线对比度；收紧标签相对车体的世界尺寸与锚点高度（§6.4 的 8px/20px 投影阈值、图集结构与两个 Draw Call 预算不变）；调整仓库/独占区地面名称的淡出区间与字号密度。所有调整只落在上述外观常量与材质/灯光工厂内，不新增 DOM 覆盖层，不以隐藏节点、路径、主状态或 L1/L2 告警语义换取观感，同步更新共置测试中的颜色/尺寸断言。
- 验证：短时性能冒烟可重复；200 台车辆相关 Draw Call≤12、默认近景总数≤150；观感调优后同一机位近/中/远景截图与原型对照留档，车辆、路径、主状态和 L1/L2 环仍全部可见；报告能够自动判定 SPEC B1～B3/B5，但只有符合 §6.1 的机器结果才能把 GATE-001 标为通过。
