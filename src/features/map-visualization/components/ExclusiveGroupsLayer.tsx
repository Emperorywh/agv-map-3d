/**
 * 独占区提示图层（SPEC §2.3、§5.1、§7.2；TASK-005）。
 *
 * 职责：把 buildExclusiveGroupsGeometry 产出的「全分组合并蓝色外沿几何」以
 *       低透明度材质挂入场景，并把分组名称锚点与图集单元 join 后合批为
 *       近景名称四边形（距离淡出：远距离隐藏、近距离显示，GPU 侧完成）。
 * 边界：外沿几何由本组件经 useMemo 构建并在卸载/视图更换时 dispose（创建者
 *       释放）；图集纹理由 Feature 根组件单一所有，本组件只消费。独占区只
 *       提供空间语义，不参与调度、不响应交互。
 * 关键不变量：
 * 1. 外沿恰好一个 Mesh（全部分组合并为一个静态 BufferGeometry，SPEC §2.3），
 *    名称恰好一个 Mesh；透明外沿不写深度，只露出比路面宽出的蓝色边缘；
 * 2. 名称按「组 id → 图集单元」join，未入图集的分组名称跳过（逐项隔离），
 *    无可用图集时名称 Mesh 整体不创建，外沿不受影响；
...
interface ExclusiveResources {
  ...
  /** 本组件创建、需在清理中释放的 geometry/material（外沿几何经 build 释放） */
  owned: { dispose(): void }[]
...
function createExclusiveResources(
  ...
  const owned: { dispose(): void }[] = [outlineMaterial]
  ...
  return { id, outline, names, owned, build }
}

/** 对称释放：外沿几何（build 拥有）、名称几何与本组件全部材质（幂等） */
