 ...
 *
 * 职责：在仿射后的平面坐标上减去固定原点并翻转 Y 得到世界 (x, z)（世界
 *       y 恒 0），并提供「平面数学角 → 世界 rotation.y」的唯一符号换算点。
 * 边界：原点由调用方（地图建模层）以「地图包围盒中心」计算后只传入一次；
 *       本模块不感知地图数据、车辆、交通资源或运行时配置。
 * 关键不变量：
 * 1. worldX = 平面x - originX、worldZ = -(平面y - originY)：渲染时平面 Y
 *    轴取负（-y），世界 y 恒为 0，不在返回值中冗余表达；
 * 2. 地图与车辆使用数学平面角（0 指向 +x，逆时针为正）。平面 y 已在映射中
 *    取负，平面方向 (cosθ, sinθ) 落到世界 (cosθ, -sinθ)，恰等于 Three.js
 *    绕 +y 旋转 +θ 后 +x 的指向（+x 转向 -z），因此 rotation.y = +平面角；
 *    该符号只在此处换算，任何业务模块不得自行再加负号；
...
export interface WorldTransform {
  ...
  /** 地图平面数学角 → Three.js rotation.y（Y 翻转后恒等于变换后平面角） */
...
export function createWorldTransform(plane: PlaneTransform, origin: PlanePoint): WorldTransform {
  ...
  return {
    ...
    toWorldXZ(x, y) {
      ...
      return { x: p.x - frozenOrigin.x, z: frozenOrigin.y - p.y }
    },
